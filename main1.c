...
int main(int argc, char **argv) {
    ...
    int m = 100; // Taille de la table de hachage

    BiblioH *bib = charger_n_entreesH(nomfic, n,m); // Charge les entrées depuis le fichier dans une bibliothèque
    ...
    enregistrer_biblioH(bib, "nouvelle_biblio.txt"); // Enregistre la bibliothèque dans un nouveau fichier
    ...
    do { 
        ...
        switch (choix) { // Structure de contrôle pour traiter le choix de l'utilisateur
            ...
            case 3: // Cas 3: Recherche par numéro
                {
                    ...
                    LivreH *ouvrage = recherche_numH(bib, numero_recherche);
                    if (ouvrage != NULL) {
                        printf("Livre trouvé :\n");
                        afficher_livreH(ouvrage);
                    } else {
            ...
            case 4: // Cas 4: Recherche par titre
                {
                    ...
                    LivreH *resultat_titre = recherche_titreH(bib, titre_recherche);
                    if (resultat_titre != NULL) {
                        printf("Livre trouvé :\n");
                        afficher_livreH(resultat_titre);
                    } else {
            ...
            case 5: // Cas 5: Recherche livres d'un auteur
            {
                ...
                LivreH *resultat_auteur = recherche_bib_auteurH(bib, auteur_recherche);
                if (resultat_auteur != NULL) {
                    printf("Livres de l'auteur '%s' :\n", auteur_recherche);
                    // Afficher les livres de l'auteur
                    while (resultat_auteur != NULL) {
                        printf("%s\n", resultat_auteur->titre);
                        resultat_auteur = resultat_auteur->suivant;
                    }
                } else {
            ...
            case 6: // Cas 6: Supprimer un ouvrage
                {
                    ...
                    if (scanf("%d %s %s", &num_suppression, titre_suppression, auteur_suppression) == 3) {
                        suppression_livreH(bib, num_suppression, titre_suppression, auteur_suppression);
            ...
            case 7: // Cas 7: Fusionner avec une autre bibliothèque
                {
                    ...
                    BiblioH *b2 = charger_n_entreesH(nom_fichier, INT_MAX,m); // Charger la deuxième bibliothèque depuis le fichier
                    ...
                    bib = fusion_bibH(bib, b2); // Fusionner la première bibliothèque avec la deuxième
                    liberer_biblioH(b2);
            ...
            case 8: // Cas 8: Recherche des ouvrages avec plusieurs exemplaires
                {
                    LivreH *doublons = recherche_dupliquesH(bib);
                    if (doublons != NULL) {
                        printf("Ouvrages avec plusieurs exemplaires :\n");
                        while (doublons != NULL) {
                            afficher_livreH(doublons);
            ...
            case 9: // Cas 9: Créer une nouvelle bibliothèque
                {
                    liberer_biblioH(bib); // Libère la mémoire de l'ancienne bibliothèque
                    bib = creer_biblioH(m); // Crée une nouvelle bibliothèque vide
            ...
            case 10: // Cas 10: Libérer la bibliothèque actuelle
                {
                    liberer_biblioH(bib); // Libère la mémoire de la bibliothèque actuelle
